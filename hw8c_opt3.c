...
printf("Implementation with more complex loop unrolling method\n");
...
void CalculateCoefficients(LinearFit *DataSet, double *A, double *B)
  {
   ...
   (*A) = (((DataSet->NextElement * (S_XY + S_XY2)) - (S_X+S_X2) * (S_Y+S_Y2)) / ((DataSet->NextElement * (S_XX+S_XX2)) - ((S_X+S_X2) * (S_X+S_X2))));
   (*B) = ((((S_XX+S_XX2) * (S_Y+S_Y2)) - ((S_XY+S_XY2) * (S_X+S_X2))) / ((DataSet->NextElement * (S_XX+S_XX2)) - ((S_X+S_X2) * (S_X+S_X2))));
